...
ULONG SoundFunc(register __a0 struct Hook *hook, register __a2 struct AHIAudioCtrl *audioctrl, register __a1 struct AHISoundMessage *smsg) {
	...
	if (port=CreatePort("IngaSoundPort", 40)) {
		...
		if (offneAHI()) {
			...
			if (audioctrl=AHI_AllocAudio(
				...
				AHI_GetAudioAttrs(AHI_INVALID_ID, audioctrl, AHIDB_Panning, &panning, TAG_DONE);
		...
		do {
			...
			WaitPort(port);
			while ((struct Message *)smsg=GetMsg(port)) {
